                    ...
                    <div class="card-tools d-flex align-items-center gap-1" style="gap:6px">
                        ...
                        @php
                            $cursoPivot   = auth()->user()->estudiante
                                ...
                                ->withPivot('estado')->first();
                            $estadoPivot  = $cursoPivot?->pivot->estado ?? 'activo';
                            $notaOk       = $data['aprobado']; // promedio >= 3.0
                            $retirado     = in_array($estadoPivot, ['retirado', 'reprobado']);
                            $puedeDescargar = !$retirado && ($estadoPivot === 'aprobado' || $notaOk);
                        @endphp
                        ...
                        @elseif($retirado)
                            <button class="btn btn-sm btn-danger ml-1" disabled>
                                <i class="fas fa-ban mr-1"></i>No disponible
                            </button>
                        @else
                            <button class="btn btn-sm btn-secondary ml-1" disabled
                                    title="Necesitas una nota final ≥ 3.0 para obtener el certificado">
                                <i class="fas fa-lock mr-1"></i>Certificado
                                <span class="badge badge-light ml-1" style="font-size:9px">Nota insuficiente</span>
                            </button>
                        @endif
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-3 col-6">
                            @php $notaFinalMostrar = $data['nota_final'] ?? $data['promedio']; @endphp
                            <div class="info-box {{ $notaFinalMostrar >= 3.0 ? 'bg-success' : 'bg-danger' }}">
                                <span class="info-box-icon"><i class="fas fa-trophy"></i></span>
                                <div class="info-box-content">
                                    <span class="info-box-text">Mi Nota Final</span>
                                    <span class="info-box-number">{{ number_format($notaFinalMostrar, 2) }}</span>
                                </div>
                            </div>
                            @if (!empty($data['nota_final_parciales']))
                                <small class="text-muted d-block mb-2">
                                    @foreach ($data['nota_final_parciales'] as $p)
                                        {{ $p['nombre'] }}: <strong>{{ $p['nota'] }}</strong>@if(!$loop->last), @endif
                                    @endforeach
                                </small>
                            @endif
                        </div>
